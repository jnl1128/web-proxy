...
// S_IRUSR == 0400 = 110000 110100 110000 110000 1010
// S_IXUSR == 0100 = 110000 110001 110000 110000 1010
...
int parse_uri(char *uri, char *filename, char *cgiargs){
  ...
  if (!strstr(uri, "cgi-bin")){ /*static file*/
    strcpy(cgiargs, "");
    strcpy(filename, ".");
    strcat(filename, uri);
    if(uri[strlen(uri)-1] == '/')
...
void serve_static(char* method, int fd, char *filename, int filesize){
  ...
  if(strcasecmp(method, "HEAD")){ 
    ...
    /*MAP_PRIVATE: indicate that this object(file) is an "copy-on-write" one.*/
    srcp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, srcfd, 0);
    // srcp = (char *)malloc(filesize);
    // rio_readn(srcfd, srcp, filesize); /*읽어온다*/
